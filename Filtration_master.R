...
lunge_rates_all <- lunge_rates_DTAG %>% 
...
Feeding_rate_h <- filtration_master %>% 
  filter(SpeciesCode %in% c("bw", "bp", "mn") & prey_general == "Krill", Phase %in% c("Day", "Twilight", "Night")) %>% 
...
Rate_by_length <- ggplot(d_strapped, 
                         aes(log10(length_distrib), log10(daily_rate))) +
