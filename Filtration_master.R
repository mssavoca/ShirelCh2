...
index = 1:10000
MRY_hyp_low_prey_dens_m3 <- rlnorm(1e4, log(1.05) + log(0.17),   log(1.8)) 
...
Feeding_rate_h <- filtration_master %>% 
  filter(SpeciesCode %in% c("bw", "bp", "mn") & prey_general == "Krill", Phase %in% c("Day")) %>% 
  group_by(ID, Species, Engulfment_L, Phase, Rate, prey_general) %>% 
  summarise(whaleLength = first(whaleLength)) %>%
  mutate(Engulfment_m3 = Engulfment_L/1000) %>%
  ungroup %>% 
...
d_strapped <- filtration_master %>% 
  ...
  group_by(Species, SpeciesCode, prey_general) %>% 
...
Daily_biomass_ingested <- ggplot(filter(d_strapped, SpeciesCode != "bb")) +
  geom_flat_violin(aes(x = fct_reorder(abbr_binom(Species), prey_mass_per_day_hyp_low_kg), y = prey_mass_per_day_hyp_low_kg/1000, 
                       fill = abbr_binom(Species)), position = position_nudge(x = 0.1, y = 0), alpha = .8, adjust = 2) +
